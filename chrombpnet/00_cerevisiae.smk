...
# root_dir = "/s/project/multispecies/fungi_code/atac/debug/schep"
root_dir = "/s/project/multispecies/fungi_code/atac/data/chrombpnet_data/s_cerevisiae"
...
rule all: 
    input: 
        ...
        os.path.join(
            ...
            "results", 
            "05_merged",
            "all_merged.sorted.shifted.bam.bai"
        ),
        os.path.join(
            root_dir, 
            "results", 
...
rule remove_duplicates:
    ...
    output: 
        dedup_grp_tmp = temp(os.path.join(
            root_dir, 
            "results", 
            "02_duplicated_marked",
            "{sample}.grouped.bam"
        )),
...
rule shift_reads:
    ...
    output: 
        ...
        sorted_bam = os.path.join(
            ...
            "results", 
            "04_shifted",
            "{sample}.shifted.sorted.bam"
        ),
        shifted_index = os.path.join(
            root_dir, 
            "results", 
            "04_shifted",
            "{sample}.shifted.sorted.bam.bai"
        )
    threads: 8
    resources: 
        mem_mb=32000
    shell:  # --ATACshift: (4 -5, 5 -4)
        """
...
rule merge_bams: 
    ...
    output: 
        merged_bam = os.path.join(
            ...
            "results", 
            "05_merged",
            "all_merged.unsorted.shifted.bam"
        ),
        merged_sorted_bam = os.path.join(
            root_dir, 
            "results", 
            "05_merged",
            "all_merged.sorted.shifted.bam"
        ),
        merged_sorted_index = os.path.join(
            root_dir, 
            "results", 
            "05_merged",
            "all_merged.sorted.shifted.bam.bai"
    ...
    shell:
        """
        rm -f {output.merged_bam} \
            && rm -f {output.merged_sorted_bam} \
            && rm -f {output.merged_sorted_index} \
...
rule remove_blacklist_peaks:
    ...
    output:
        tmp_bed = temp(
            os.path.join(
                ...
                "results", 
                "06_peaks", 
                "all_merged_relaxed_peaks_blacklist_slopped.tmp.bed"
            )
        ),
        filtered_bed = os.path.join(
            root_dir, 
            "results", 
            "06_peaks", 
            "all_merged_relaxed_peaks_no_blacklist.bed"
    ...
    shell:
        """
        bedtools slop -i {input.blacklist} \
        -g {input.chrom_sizes} \
        -b 1057 \
        > {output.tmp_bed}; \

        bedtools intersect -v -a {input.peaks} \
        -b {output.tmp_bed} \
        > {output.filtered_bed}
        """
